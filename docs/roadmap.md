...
Allow modules to expose type aliases and opaque types alongside values. A file can declare `type Point = {x: I64, y: I64}` or `type UserId = opaque I64` and the importing module gets the name in scope for use in annotations and type checking. Currently `import` only binds a value; the type environment is not propagated. The fix is to carry a type environment alongside the value in module compilation, and merge it into the caller's type env on import. Types are erased at codegen — this is a type-checker-only change. Natural prerequisite for opaque types being useful across module boundaries (otherwise the `wrap`/`unwrap` restriction can't be enforced from outside).
...
## Tensor types with dimension checking
